...
register_activation_hook( __FILE__, 'SEORedirect301s\seo_redirect_301_activate' );

add_action('admin_menu', 'SEORedirect301s\register_seo_redirect_301_page');
...
add_action("admin_init", "SEORedirect301s\seo_redirect_301_register_style_scripts");
...
add_action( 'admin_init', 'SEORedirect301s\register_seo_redirect_301_install_dependency_settings' );
...
add_action( 'save_post', 'SEORedirect301s\seo_redirect_save_current_slug' );
...
add_action('template_redirect', 'SEORedirect301s\seo_redirect_slt_theme_filter_404');
...
function seo_redirect_slt_theme_filter_404() {
  ...
  if (!preg_match("/\?/", seo_redirect_curl_page_url())) {
    ...
    if (!is_front_page()) {
      ...
      $post_template_name = "";
      $gcu =  $abcTom->get_current_url();
      $gcu = str_replace(":443","",$gcu);
      $page_slug = str_replace(get_option("siteurl"), "", $gcu);
      ...
      if ((($template_name == "" && $post_template_name == "") || !in_array($wp_query->post->post_type, $acceptable_values))) {
        ...
        $row = $abcTom->get_row("slug_history", "*", "post_id <> 0 AND url='".sanitize_url($seo_redirect_curl_page_url)."/'");
...
if (isset($_GET["post"]) && $_GET["post"] != "") {
  add_action( 'add_meta_boxes', 'SEORedirect301s\seo_redirect_admin_page_widget_box' );
}
function seo_redirect_admin_page_widget_box() {
  $abcTom = new TomM8();
  if (isset($_GET["delete_url"]) && isset($_GET["post"]) && check_admin_referer( 'delete-slug-301-redirect'.sanitize_text_field($_GET["post"]))) {
    $record = $abcTom->get_row("slug_history", array("post_id", "url"), "post_id=".sanitize_text_field($_GET["post"])."&url='".sanitize_url($_GET["delete_url"])."'");
    // Check if slug history record exists
    if ($record) {
      // slug history record does exist so attempt to delete it.
      global $wpdb;
      $wpdb->delete($wpdb->prefix."slug_history", array("post_id" => sanitize_text_field($_GET["post"]), "url" => sanitize_url($_GET["delete_url"])), array('%d','%s'));
    }
  }
  ...
  foreach ($screens as $screen) {
      add_meta_box(
          ...
          'SEORedirect301s\seo_redirect_inner_custom_box',
          $screen
      );
  }
}
...
function seo_redirect_inner_custom_box( $post ) {
  ...
  $nonce = wp_create_nonce( 'delete-slug-301-redirect'.$post->ID );
  ?>
  <script>
  function updateAndRefresh() {
    ...
    jQuery('.components-button.editor-post-publish-button.editor-post-publish-button__button.is-primary,#publish').click();
    let c=setInterval(function() {
      if (!jQuery('.components-button.editor-post-publish-button.editor-post-publish-button__button.is-primary,#publish').hasClass('is-busy')) {
        jQuery("table.slug-history tbody").append("<tr><td><a target='_blank' href='"+url+"'>"+url+"</a></td><td><a class='delete' href='/wp-admin/post.php?post="+currentId+"&action=edit&delete_url="+url+"&_wpnonce=<?php echo($nonce); ?>"+url+"'>Delete</a></td></tr>");
  ...
  <p>
    ...
    <span style="background: #cac9c9; padding: 10px;">
      <?php echo(esc_html(get_option("siteurl"))); ?>/<input type="text" name="seo_redirect_url" id="seo_redirect_url" />
    ...
    <tbody>
      <?php
        ...
        <?php if ((get_permalink($redirect->post_id) != "") && (preg_replace("/\/$/", "", $redirect->url) != preg_replace("/\/$/", "", get_permalink($redirect->post_id)))) {
          ...
          ?>
          <tr>
            <td><a target="_blank" href="<?php echo(esc_url($redirect->url)); ?>"><?php echo(esc_url($redirect->url)); ?></a></td>
            <td><a class="delete" href="<?php echo(esc_url(get_option("siteurl"))); ?>/wp-admin/post.php?post=<?php echo(esc_html($redirect->post_id)); ?>&action=edit&delete_url=<?php echo(esc_url($redirect->url)); ?>&_wpnonce=<?php echo(esc_html($nonce)); ?>">Delete</a></td>
          </tr>
        <?php } ?>
      <?php } ?>
    </tbody>
...
add_action( 'save_post', 'SEORedirect301s\seo_redirect_save_postdata' );
/* When the post is saved, saves our custom data */
function seo_redirect_save_postdata( $post_id ) {

  // First we need to check if the current user is authorised to do this action.
  if ( 'page' == sanitize_text_field($_REQUEST['post_type']) ) {
  ...
  if (sanitize_text_field($_POST['seo_redirect_url']) != "") {
    $abcTom = new TomM8();
...
add_action( 'wp', 'SEORedirect301s\seo_redirect_301_setup_schedule' );
/**
...
add_action( 'seo_redirect_301_daily_event', 'SEORedirect301s\seo_redirect_301_do_this_daily' );
/**
